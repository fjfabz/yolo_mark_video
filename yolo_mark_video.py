...
def write_txt_img():
    ...
    # write txt
    bounding_str = "0 {} {} {} {}".format(mousex / img_width, mousey / img_height, bound_width / img_width,
...
def click_and_mark(event, x, y, flags, param):
    ...
    mousey=y
    clone = frame.copy()
    cv2.putText(clone, '[space]:mark, [s]:stop, [1]:big, [2]:small, [esc]:next', (10, img_height-20), cv2.FONT_HERSHEY_PLAIN, 1.2,
...
def mark_process(file_path):
    global frame,clone,outfolder,img_height,img_width,name,bound_height,bound_width
    ...
    while True:
        ...
        if k % 256 == 27:
            ...
            write_txt_img()
        elif k % 256 == 49:  # 1 pressed
            bound_width *= 1.5
            bound_height *= 1.5
        elif k % 256 == 50:  # 2 pressed
            bound_width /= 1.5
            bound_height /= 1.5
        elif k % 256 == 51:  # 3 pressed
            bound_width *= 1.5
            #bound_height *= 1.5
        elif k % 256 == 52:  # 4 pressed
            bound_width /= 1.5
            #bound_height /= 1.5
        elif k % 256 == 53:  # 5 pressed
            #bound_width *= 1.5
            bound_height *= 1.5
        elif k % 256 == 54:  # 6 pressed
            #bound_width /= 1.5
            bound_height /= 1.5
